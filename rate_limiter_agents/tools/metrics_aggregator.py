...
def build_error_summary(
    logs: list, window_minutes: int = 15, per_ip_address: bool = False
) -> dict[str, Any]:
    total = len(logs)
    blocked = sum(1 for log in logs if log.was_blocked)
    block_rate = (blocked / total * 100) if total > 0 else 0.0

    code_counts = Counter(log.response_code for log in logs if log.response_code)
    ...
    error_logs = [log for log in logs if log.response_code and log.response_code >= 400]
    top_error_paths = [
        {"path": p, "count": c}
        for p, c in Counter(
            log.request_path for log in error_logs if log.request_path
        ).most_common(5)
    ]
    top_block_reasons = [
        {"reason": r, "count": c}
        for r, c in Counter(
            log.reason for log in logs if log.was_blocked and log.reason
        ).most_common(5)
    ]
    ...
    if per_ip_address:
        ip_blocks = Counter(
            log.client_ip for log in logs if log.was_blocked and log.client_ip
        )
        ...
        result["ip_concentration_pct"] = (
            round(top_ip_count / blocked * 100, 2) if blocked else 0.0
        )

    return result


def build_token_health_summary(
    logs: list, window_minutes: int = 15, per_ip_address: bool = False
) -> dict[str, Any]:
    total = len(logs)
    tokens = [log.remaining_tokens for log in logs if log.remaining_tokens is not None]
    ...
    for log in logs:
        if log.request_path and log.remaining_tokens is not None:
            path_tokens[log.request_path].append(log.remaining_tokens)
    ...
    if per_ip_address:
        ...
        for log in logs:
            if log.client_ip and log.remaining_tokens is not None:
                ip_tokens[log.client_ip].append(log.remaining_tokens)

        ips_near_depletion = sum(
            1 for tks in ip_tokens.values() if any(0 < t <= threshold for t in tks)
        )
        ips_depleted = sum(1 for tks in ip_tokens.values() if any(t == 0 for t in tks))
...
def build_top_paths_summary(
    logs: list, window_minutes: int = 60, per_ip_address: bool = False
) -> dict[str, Any]:
    total = len(logs)

    path_stats: dict[str, dict] = defaultdict(
        lambda: {"total": 0, "blocked": 0, "methods": Counter()}
    )
    for log in logs:
        if log.request_path:
            path_stats[log.request_path]["total"] += 1
            if log.was_blocked:
                path_stats[log.request_path]["blocked"] += 1
            if log.http_method:
                path_stats[log.request_path]["methods"][log.http_method] += 1

    top_by_traffic = sorted(
        path_stats.items(), key=lambda x: x[1]["total"], reverse=True
    )[:10]
    top_by_block = sorted(
    ...
    if per_ip_address:
        ...
        for log in logs:
            if log.client_ip and log.request_path and log.was_blocked:
                ip_path_blocks[log.request_path][log.client_ip] += 1
        ...
        result["unique_ips"] = len(set(log.client_ip for log in logs if log.client_ip))
